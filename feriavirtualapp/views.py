...
def connection():
    s = '54.145.168.187\EC2AMAZ-BDN65VE\SQLEXPRESS01,14334' #Your server name 
    d = 'maipo-grande' 
    u = 'sa' #Your login
...
def solicitudesRevisor(request):
    cart = Cart(request)
    solir = Post.objects.filter(EstadoSolicitud__in=("3","6","8"))
    context ={'solir':solir}
        ...
        if form.is_valid():
            ...
            if estadoactual == "1":
                ...
                if len(topProductos1) == 1:
                    ...
                    prodganadores= topProductos1.objects.filter(precio=min_precio)
...
def elimProducto(request, id):
    conn = connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM dbo.feriavirtualapp_producto WHERE id = ?", id)
    conn.commit()
    conn.close()
    return redirect('mis-productos')
